...
//    for (entry <- values) {
//      val fields = entry.split("[\\s+]")
//      writer.put(table.getEntityId(fields(1)), "family", "column1", fields(0).toLong, fields(2))
//      writer.flush()
//    }
//  }
  ...
  test("An import job with multiple timestamps imports all timestamps in local mode.") {
    ...
    // Build test job.
    new MultipleTimestampsImportJob(uri, source).run
...
//  }
  ...
  test("an import job that writes to a Kiji table with timestamps is run using local mode") {
    ...
    val source = IterableSource(importWithTimeInput, new Fields("line"))

    // Build test job.
//    new ImportJobWithTime(source).run
//    .arg("input", "inputFile")
//    .arg("output", uri)
...
//              new KijiColumnName("info:fullname"), AvroValidationPolicy.NONE
//          ).build()
//
//      // Create test Kiji table.
//      doAndRelease {
...
object KijiSourceSuite {
  ...
  class MultipleTimestampsImportJob(uri: String, source: IterableSource[String])
      extends KijiJob() {
    // Setup input.
    source
        .read
   ...
   */
  class ImportJobWithTime(source: IterableSource[(Long, String)]) extends KijiJob() {
    // Setup input.
//    source
//        .read
//        // Generate an entityId for each line.
//        .map(('timeStamp, 'line) -> 'entityId) { EntityId(_: String) }
//        // Write the results to the "family:column1" column of a Kiji table.
//        .write(KijiOutput.builder
//            .withTableURI(args("output"))
//            .withTimestampField('offset)
